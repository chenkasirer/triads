...
const Fretboard: React.FC<FretboardProps> = ({ positions }) => {
  ...
  return (
    <div className="bg-white rounded-lg p-2 sm:p-4 shadow-lg w-full">
      {/* Mobile: horizontal scroll, Desktop: no scroll needed */}
      <div className="overflow-x-auto sm:overflow-x-visible w-full">
        <svg 
          width={width} 
          height={height} 
          className="mx-auto sm:max-w-full block"
        ...
        >
          ...
          {/* Fret markers */}
          {/* Single dots on frets 3, 5, 7, 9 */}
          {[3, 5, 7, 9, 15, 17, 19, 21].filter(f => f <= frets).map(fret => (
            <circle
              key={`marker-${fret}`}
